...
const Home = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/menu?category=${encoded}`)
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <div className="user-header">
        <h1>{getGreeting()}{userInfo.name ? `, ${userInfo.name.split(' ')[0]}` : ''}</h1>
      ...
      <div className="bottom-nav">
        ...
        <button className="bottom-nav-item" onClick={() => navigate('/cart')} style={{ position: 'relative' }}>
          ...
          {cartCount > 0 && (
            <span style={{
              position: 'absolute', top: 2, right: 14, background: 'red', color: 'white',
